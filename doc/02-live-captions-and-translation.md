...
[`SpeechVocabularySettings`](../Sources/Capture/SpeechVocabularySettings.swift) supplies 41 default terms on first launch, then persists the user's entire saved vocabulary in UserDefaults. Vocabulary settings accepts one word or phrase per line. Save removes blank lines and surrounding whitespace and deduplicates case-insensitively. An explicitly saved empty vocabulary stays empty rather than restoring defaults.

With AI configured, the Vocabulary tab offers Generate from Markdown. It opens the native picker directly; selecting files starts the task and opens an independent `Window` Scene for progress, errors, and review. Cancelling the initial picker creates neither a window nor a task. `AppDelegate` owns `VocabularyImportController`, so changing Settings tabs or closing Settings leaves generation running. The generation window owns the operation's lifetime: closing it cancels and discards unsaved review content. Stop cancels current and pending requests but keeps the window and generated candidates for review/save; without candidates it returns to file selection. Retry Incomplete Requests retries failed, interrupted, and unsent requests without repeating successful ones.
...
Each successful request must return at most 50 specific named entities under strict JSON Schema, each valuable for English ASR. Generic technical concepts, feature categories, process terms, and ordinary phrases are excluded. Filenames are not sent; contents, excerpts, and unsaved candidates stay in memory. Each success is immediately deduplicated against saved vocabulary and the current settings draft before appending editable checkboxes. Filenames and excerpts expand on demand. Later results cannot overwrite user edits or deselections. Add and Save deduplicates again, writes only selected new terms to UserDefaults, and adds those terms to the settings draft without submitting or replacing unrelated manual changes. The same window shows save feedback. Failed requests and per-attempt timing are expandable; even an all-failed operation can be retried.

Starting or resuming a meeting freezes the saved vocabulary in `CaptureCoordinator` and passes the same snapshot to both recognizers. Editing vocabulary during recording cannot change those recognizers; changes take effect at the next English start/resume. Chinese recognition does not use the English vocabulary.

Custom model directories are separated by locale and a SHA-256 vocabulary fingerprint. Prepared models are reusable for identical vocabulary; changed vocabulary creates a new configuration. Apple Speech training data and model files stay local. AI reuses saved vocabulary differently: refinement freezes the complete list at operation start, insights send only recent-context matches, and titles send none. These AI uses do not change local recognition's privacy boundary; see [AI insights and refinement](04-ai-insights-and-refinement.md).
