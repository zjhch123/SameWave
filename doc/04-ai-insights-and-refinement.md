...
- Settings uses native sheets over the current main/preparation surface, including menu and Command-comma entry points. AI configuration links select AI Services. AISettingsDraft belongs to SettingsNavigation, retaining preference edits across tab switches and Settings dismissal. Save commits connection and context-window preferences; Cancel restores those draft values. Neither action reverts the immediate master switch or changes vocabulary.
- The draft owns connection tests, model discovery, cancellation tokens, and loading/error state. Changing provider/key/address invalidates both checks and clears discovered models; changing the model invalidates its connection test. Settings dismissal cancels checks even if SwiftUI retains the dismissed view, releases loading state, and rejects late replies. Discovery preserves any model typed during the request. Vocabulary extraction has a separate owner and continues across dismissal. The master switch cancels all these owners synchronously through app assembly, independently of view lifetime; cancellation tokens reject late completions.
- AI Services places its master switch first with one sentence explaining immediate cancellation and configuration retention. Connection and token-limit help stays concise; detailed request and data behavior lives in this guide. Configuration actions select this tab without submitting vocabulary drafts. When AI is off, entry points say Enable AI Services. Custom settings scroll while Save/Cancel stay fixed at the bottom.
...
- While the master switch is on, a draft can run a representative connection test containing nested objects and arrays, verifying the actual contract instead of accepting an accidentally valid simple object.
