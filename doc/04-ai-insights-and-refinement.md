...
AI Services settings is shared by insights, refinement, titles, and Markdown vocabulary generation. Insights is a consumer, not the owner or proxy for other features' configuration. The app-wide Enable AI Services switch defaults on and persists independently of connection configuration. AI generation requires both an enabled switch and valid saved configuration. Turning it off applies immediately, cancels active and queued work across all meetings, and blocks new generation, automatic ticks, extraction retries, connection tests, and model discovery. Turning it back on permits normal triggers; canceled manual operations require an explicit retry. Existing recording schedules continue, so future eligible automatic ticks can resume without restarting capture. Credentials, connection preferences, saved results, unsaved insight results, and extracted terms remain intact. Local capture, translation, history, export, manual vocabulary editing, and Retry Save remain available.
...
- Settings uses native sheets over the current main/preparation surface, including menu and Command-comma entry points. AI configuration links select AI Services. Every preference edit writes directly to `AISettings` and its persistence boundary, including while AI is off. There is no connection draft, Save, Revert, or unsaved-change footer. Done closes all tabs; closing is not a persistence event.
- The context-window preference retains the visible text in UserDefaults. Its parsed value exists only for whole integers from 16,384 through 2,000,000, accepting correctly grouped commas. Incomplete, malformed, overflowing, and out-of-range input remains visible across relaunch and blocks new AI requests instead of using a previous hidden numeric value. Other fields still save independently. Existing requests retain their operation-start configuration and token budget.
...
- While the master switch is on, fetch model lists from sibling `/models` with the same Bearer key. Successful results populate a menu while keeping manual model entry. A 404/405, empty list, or nonstandard response does not block configuration.
