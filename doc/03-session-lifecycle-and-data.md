...
For an existing persistent file, history reads Core Data metadata in read-only mode before constructing the SwiftData container. Its entity names must exactly match the current six-model schema. A different/missing entity set reports a localized storage failure; corrupt-file errors propagate unchanged. Failed preflight never replaces the file or falls back to an empty store. See the [storage incident analysis](history-storage-incident-2026-09-09.md) for the reproduced automatic-migration failure this prevents.
...
New Meeting saves a draft immediately, selects it, and displays preparation. Quick Start creates the same draft before starting capture, without requiring documents, custom prompts, or AI. Both creation paths copy the current Settings > Insights defaults into independent meeting definitions with fresh UUIDs, retaining each title, prompt, focus, and automatic-update choice. An unset preference supplies one editable Meeting Overview with automatic updates off; a saved empty list stays empty. Loading, starting, or resuming an existing meeting never reapplies defaults. Default edits cannot change existing definitions or snapshots. Unreadable defaults remain preserved and block new draft creation with a visible error until explicitly reset in Settings.
...
- The sidebar Delete action first presents a native confirmation alert naming the meeting and the transcript, documents, vocabulary, and insights that will be permanently removed. The title, removal warning, and buttons follow the active app language; the meeting name remains verbatim. The native Cancel action keeps its Escape shortcut; Return has no default action. Cancelling or dismissing changes neither the meeting nor selection. Confirming uses the existing deletion boundary. Deleting the displayed meeting selects another mounted meeting if present, otherwise the most recently created remaining meeting. Deleting another meeting preserves selection. The selected UUID persists immediately. When no meetings remain, clear the transcript and show a real empty state without a capture dock or synthetic sidebar row; New Meeting explicitly creates a draft.
