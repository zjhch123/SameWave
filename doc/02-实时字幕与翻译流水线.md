...
3. 英文首次使用时，把硬编码的 41 个产品名、缩写和人名作为统一 `count = 1` 的 `PhraseCount` 生成 `SFCustomLanguageModelData`，在本地编译为语言模型与词表文件后，通过 `customizedLanguage` content hint 交给识别器。本实验不提供自定义读音，也不设置自定义模型权重。
4. 检查并安装当前 transcriber 所需的系统语音资源。
5. 查询 `SpeechAnalyzer.bestAvailableAudioFormat`。
6. 创建 `AnalysisContext`，同时把这 41 个词写入 `.general` contextual strings；系统音频和麦克风两路使用同一份上下文。词表只改变模型的候选概率，不在结果上做字符串替换，也没有用户配置或持久化入口。
