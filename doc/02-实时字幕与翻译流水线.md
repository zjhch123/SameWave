...
自定义模型目录按 locale 与词表内容的 SHA-256 指纹隔离。同一词表可复用已经准备的模型，词表发生变化则生成新的模型配置，不会误用旧缓存。用于 Apple Speech 的训练数据和模型文件留在本机。用户明确发起会后 AI 优化时，操作开始时已保存的词表会作为专有词提示随 transcript 发送给所选服务商；这不改变实时识别的本地边界，详见 [`04-AI洞察与会后优化.md`](04-AI洞察与会后优化.md)。
