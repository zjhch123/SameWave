...
两路音频各有一个 [`NativeSpeechEngine`](../Sources/Capture/NativeSpeechEngine.swift)：

1. 请求语音识别授权。
2. 创建指定 locale 的 `SpeechTranscriber(preset: .progressiveTranscription)`。
3. 检查并安装系统语音资源。
4. 查询 `SpeechAnalyzer.bestAvailableAudioFormat`。
5. 建立 `AsyncStream<AnalyzerInput>` 并启动 analyzer。
6. `feed` 将采集器采样转换成 analyzer 格式后 yield。
7. 结果流将非 final 发送到 `onInterim`，final 发送到 `onCommit`。

[`CaptureCoordinator.makeEngine(for:)`](../Sources/Meeting/CaptureCoordinator.swift) 在创建回调时绑定 `.remote` 或 `.mine`，所以 ASR 结果天然带说话人。

## 4. Section 数据模型

[`Section`](../Sources/Meeting/CaptionStore.swift) 是 UI、翻译和持久化的共同实时单位：
