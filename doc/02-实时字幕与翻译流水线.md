...
应用身份为 `com.plus.samewave`，词表使用该身份的 UserDefaults 域；自定义语音模型使用 `Application Support/SameWave/SpeechLanguageModel` 目录及 `com.plus.samewave.terms` 标识。旧身份的词表和模型缓存不自动读取或迁移，旧数据文件不删除。
