...
- Leave manual vocabulary edits and an AI Services draft unsaved, then save selected generated terms. Only selected terms persist; manual and AI drafts remain pending. Reopen Settings and use Revert in Connection; saved vocabulary must remain unchanged. Test manual multiline Add, saved-row Save/Cancel, Remove, duplicate feedback, and all-or-nothing invalid-line handling.
...
## Settings action ownership smoke checks

Issue #32 (2026-09-10): XcodeGen, unsigned Debug build, and source/link/extracted-localization audits passed. The `SameWave` scheme passed 250/250 full English/US tests and 25/25 selected Simplified Chinese/CN tests on `platform=macOS,arch=arm64`; the final focused English footer-render check also passed. Native renders were inspected in both appearances. The signed desktop app was replaced and launched with `./build.sh`; strict signature and installed-process checks passed. Desktop interactions verified Done on all three tabs, switch persistence without Save, invalid-draft retention after reopening, both pending-AI links, local Save Changes/Revert without dismissal, and scrolled access to the fixed footer. A temporary context-window change persisted and was restored to its original value, and the original AI enablement was restored. No audio capture or real AI request was needed for this settings-only change.

For issue #32, open General, AI Services, and Vocabulary at 600×540 in English and Simplified Chinese. Every tab has one fixed Done action. With no connection changes, Save Changes and Revert are disabled while Done remains available. Toggle AI off/on and close/reopen: the preference persists without saving the connection.

Edit a connection field and use Save Changes: values persist, the draft becomes clean, and Settings stays open. Edit again and use Revert: only connection fields return to their saved values. Leave an invalid context window, switch to General or Vocabulary, and use the pending-AI footer link to return. Done, Escape, and an unconsumed Return close without committing or discarding it; reopen and verify the exact text. Return inside a connection text field ends editing first. A vocabulary row's Escape cancels that row before the sheet closes; multiline Return remains local. Test and discovery cancellation still occur on dismissal; vocabulary extraction keeps its separate lifetime.

Check empty, nonnumeric, fractional, malformed grouping, and values outside 16,384–2,000,000: they must block connection saving, never silently retain the previous numeric value. A successful connection test on a dirty draft must still report that it is not saved. Scroll a custom-provider form to its actions while the common Done and pending-draft notice remain visible. No real provider request is needed to verify closing and persistence.
