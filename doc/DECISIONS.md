...
## DEC-20260905-005：开发完成后默认覆盖安装并启动桌面版

- **日期**：2026-09-05
- **状态**：Accepted
- **范围**：本项目开发交付流程与持续授权
- **背景**：仅完成构建和测试后，用户还需另行要求覆盖桌面版并启动；用户明确要求将安装启动作为每次开发的默认收尾步骤。
- **决定**：在 `MEMORY.md` 保存该偏好，由 `AGENTS.md` 要求任务开始时读取。每次开发和必要验证完成后，默认运行 `./build.sh`，再检查签名及桌面版进程，无需重复确认；遵循用户在具体任务中的明确例外。
- **未采用方案**：只记在当前对话或每次重新请求安装授权，无法保证后续任务按相同流程交付。
- **理由与权衡**：复用既有脚本和签名配置，让用户直接体验开发结果；安装会替换并重启桌面版，这是用户已明确授权的流程。
- **验证与影响**：前一轮已成功运行该脚本，桌面版签名校验和进程检查通过。本次只更新记忆与文档，未改动应用或安装脚本；开发指南与协作规则同步删除默认不安装的约束。
- **相关文件**：`MEMORY.md`、`AGENTS.md`、`doc/06-开发与验证指南.md`、`build.sh`。

---

## DEC-20260905-004：启动恢复用户上次选择，不再自动选中最新未结束会议

- **日期**：2026-09-05
- **状态**：Accepted
- **范围**：主窗口会话选择、启动恢复与会话切换

### 背景与决定

历史选择原本只存在于 `MainView` 的内存状态中，而启动恢复会直接挂载最新未结束会议。用户希望重开后继续查看上次选择的对话，因此需要将“恢复未结束记录”和“决定当前显示哪条记录”分开。

- 使用 UserDefaults 的 `selectedMeetingID` 保存实际显示的会议 UUID；选择新会议时移除该键。历史模型和 transcript 继续由 SwiftData 保存。
- 选择状态由协调器与会话生命周期一起更新，View 绑定同一份状态；历史选择优先于后台挂载会话。切换、结束和删除成功后才更新选择，失败时保留原选择。
- 启动时把未结束记录统一规范为暂停，再恢复所选记录。已结束记录直接显示历史，未结束记录只挂载为暂停。选择缺失、无效或指向已删除记录时显示新会议；数据库错误保留选择键并显示错误。

### 未采用方案与权衡

- **继续优先选中最新未结束记录**：会覆盖用户查看历史或选择新会议的意图。未选中的暂停记录仍保留在侧栏，可手动恢复。
- **仅在退出或 View 刷新时保存**：不能稳定覆盖窗口关闭、挂载会话之间切换和异常退出；因此在选择与挂载记录变化时即时保存。
- 只持久化稳定 UUID，不保存 SwiftData 对象或复制会话内容，也不增加新的数据库字段或兼容路径。

### 验证与影响

- XcodeGen、无签名 Debug 构建通过，生成的 plist/entitlement 无差异；`SameWave` scheme、`platform=macOS,arch=arm64` 全量 XCTest 100/100 通过。
- 新增 10 项测试，覆盖重建协调器后的历史/暂停会话恢复、无效与已删除记录、新会议、连续挂载切换、历史查看优先级、删除和结束。使用隔离 UserDefaults 与内存 SwiftData，不启动音频或真实翻译。
- 已按用户要求运行 `build.sh`，使用项目既有证书覆盖安装桌面版并启动；安装产物的严格签名校验通过，已确认桌面版进程运行。当前没有可用的原生界面自动化，实际退出重开后的侧栏、舞台和洞察面板一致性仍需按开发指南在安装版中人工验收。
- 相关文件：`Sources/Meeting/CaptureCoordinator.swift`、`Sources/History/MeetingHistory.swift`、`Sources/App/MainView.swift`、`MeetingSidebar.swift`、`MeetingStage.swift`、`SameWaveApp.swift`、`Tests/MeetingSelectionTests.swift`、README 与架构/会话/验证 reference。
