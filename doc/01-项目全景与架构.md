...
| 语音识别 | Speech `SpeechAnalyzer` + `DictationTranscriber` / `SpeechTranscriber` | 双路本地流式 ASR；英文使用内置词表模型 |
...
- [`Sources/App/MeetingCaptionsApp.swift`](../Sources/App/MeetingCaptionsApp.swift)
  - 建立主窗口、设置窗口和菜单栏入口。
  - `AppDelegate` 创建并连接 `CaptureCoordinator`、`MeetingHistoryStore`、`InsightSettings`、`InsightEngine` 和 `TranscriptRefiner`。
...
- [`Tests/InsightEngineTests.swift`](../Tests/InsightEngineTests.swift)：AI 上下文窗口和宽容 JSON 解析。
