...
| 领域 | 当前选择 | 作用 |
|---|---|---|
| UI | SwiftUI + Observation | 三栏窗口、实时响应状态 |
...
| 语音识别 | Speech `SpeechAnalyzer` + `SpeechTranscriber` | 双路本地流式 ASR |
| 翻译 | Translation `TranslationSession` | 本地英译中 |
| 持久化 | SwiftData | 会议、逐段文字、洞察和优化稿 |
| 云端 AI | `URLSession` + OpenAI `/chat/completions` | 可选洞察和会后优化 |
...
flowchart LR
    ...
    SCK --> ASR1[NativeSpeechEngine\nremote]
    MC --> ASR2[NativeSpeechEngine\nmine]
...
- [`Sources/App/MeetingCaptionsApp.swift`](../Sources/App/MeetingCaptionsApp.swift)
  - 建立主窗口、设置窗口和菜单栏入口。
  - `AppDelegate` 创建并连接 `CaptureCoordinator`、`MeetingHistoryStore`、`InsightSettings`、`InsightEngine`。
...
- [`Sources/Capture/NativeSpeechEngine.swift`](../Sources/Capture/NativeSpeechEngine.swift)：采样率转换、SpeechAnalyzer 输入流、interim/final 回调。
...
- [`Sources/Meeting/CaptionStore.swift`](../Sources/Meeting/CaptionStore.swift)：Section、说话人、抢话权、原文和翻译状态。
- [`Sources/Meeting/TranslationBridge.swift`](../Sources/Meeting/TranslationBridge.swift)：每个 Section 只保留最新待翻请求，并在 Section 间公平调度。
...
- [`Sources/Insights/InsightEngine.swift`](../Sources/Insights/InsightEngine.swift)：实时触发、合并、历史单次生成、批量会后优化。
...
- [`Sources/App/MainView.swift`](../Sources/App/MainView.swift)：左侧历史、中间文字、右侧洞察三栏布局和控制 dock。
...
│ 新会议        │ 发言人  14:32                    │ 当前话题            │
│ 录制中…       │ 中文译文（主视觉）                 │ 参考回答            │
│ 已暂停        │ English source（次要）            │ 建议 / 待办 / 决定   │
│ 已结束历史    │                                  │                    │
│              │       [语言][麦克风][暂停][结束]   │                    │
└──────────────┴──────────────────────────────────┴────────────────────┘
```
...
“本地”只适用于实时字幕主链路：音频、ASR、Apple Translation 和 SwiftData 都在本机。AI 洞察与会后优化会将 transcript 文本发送给用户选择的第三方接口。设置页已经明确提示这一区别，文案和产品对外描述也应持续保持一致。
