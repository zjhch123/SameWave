...
正式英文名为 SameWave：工程与 scheme 为 `SameWave`，测试 target 为 `SameWaveTests`，应用标识为 `com.plus.samewave`，入口类型为 `SameWaveApp`。中文显示名、可执行文件和 Swift 模块仍为“同频”。
...
| 云端 AI | `URLSession` + OpenAI `/chat/completions` | 可选洞察、会后优化、会议标题和 Markdown 词表提取 |
...
`SameWave.xcodeproj` 是由 `project.yml` 生成的本地产物，不纳入版本控制，也不应被手工维护。
...
flowchart LR
    ...
    VS --> IE
    VS --> TR
    MD[用户选择的 Markdown 正文] --> VI[VocabularyImportController]
...
- [`Sources/App/SameWaveApp.swift`](../Sources/App/SameWaveApp.swift)
  - 建立主窗口、设置窗口、Markdown 词表生成窗口和菜单栏入口。
  - `AppDelegate` 创建并连接 `CaptureCoordinator`、`MeetingHistoryStore`、`AISettings`、`InsightEngine`、`TranscriptRefiner`、`MeetingTitleGenerator`、`SpeechVocabularyDraft` 和 `VocabularyImportController`。
  - 启动时请求语音/麦克风权限，并恢复上次选中的会议；未结束记录统一为暂停，无有效选择时显示新会议。
...
- [`Sources/Capture/SpeechVocabularySettings.swift`](../Sources/Capture/SpeechVocabularySettings.swift)：英文词表的默认值、规范化与 UserDefaults 持久化，以及应用持有的 `SpeechVocabularyDraft`；导入仅保存审核选中的新增词并补入草稿，设置页其他未保存修改保持独立。
...
- [`Sources/AI/LLMProvider.swift`](../Sources/AI/LLMProvider.swift)：最小 provider 协议、错误类型和服务商配置。
- [`Sources/AI/OpenAICompatibleProvider.swift`](../Sources/AI/OpenAICompatibleProvider.swift)：唯一 HTTP 实现。
- [`Sources/AI/AISettings.swift`](../Sources/AI/AISettings.swift)：全应用唯一的 UserDefaults + Keychain AI 配置及统一可用性校验。
- [`Sources/AI/JSONResponseParser.swift`](../Sources/AI/JSONResponseParser.swift)：各 AI 用例共用的严格 JSON 解码边界。
- [`Sources/Capture/VocabularyImportController.swift`](../Sources/Capture/VocabularyImportController.swift)：应用级导入状态、逐请求结果与耗时、停止/重试编排和选择性保存。
- [`Sources/Capture/VocabularyImportWindow.swift`](../Sources/Capture/VocabularyImportWindow.swift)：独立窗口的简短进度、可编辑勾选审核、来源披露及请求详情；实际窗口关闭取消并丢弃未保存结果。
...
- [`Sources/App/SettingsView.swift`](../Sources/App/SettingsView.swift)：设置 Tab 与跨窗口导航；AI 功能的配置入口直接选中“AI 服务”。
- [`Sources/AI/AISettingsView.swift`](../Sources/AI/AISettingsView.swift)：全应用 AI 服务配置草稿、模型查询、连接测试和保存。
- [`Sources/Capture/SpeechVocabularySettingsView.swift`](../Sources/Capture/SpeechVocabularySettingsView.swift)：逐行编辑、保存或取消英文识别词表；点击 Markdown 入口时先直接选择文件，选中后再打开独立生成窗口。
...
- [`Sources/Resources/SameWave.entitlements`](../Sources/Resources/SameWave.entitlements)：麦克风等运行时权限声明。
...
- 设置窗口使用“AI 服务”和“词表”两个 Tab；前者统一服务于洞察、会后优化、标题和词表生成。两个页面都采用显式保存/取消，不在输入时即时修改运行时配置。各功能的“配置 AI 服务”入口直接定位 AI Tab，跳转不提交词表草稿。Markdown 首次文件选择由词表 Tab 直接发起，选中后的生成和审核属于独立小窗，任务状态不依赖 Settings Scene 的生命周期。生成窗口可边生成边审核，选中的新词直接保存，其他手动设置草稿不会被连带提交。
