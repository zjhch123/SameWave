...
App identity, persisted IDs, recognition/translation language values, prompt source text, export templates, and generated meeting titles remain independent of interface language. New AI Insights use the active app language for conclusions, points, and summary items; all request paths share this language contract and context preflight counts the actual system prompt. Saved insight versions retain their original text; regeneration after a language change appends a new version. Editable meeting/insight titles, documents, vocabulary, transcripts, and saved results render verbatim. The initial default overview title is initialized in the current app language. Saved default titles are editable content; new meetings copy their exact text. The fixed full-summary title and section headings are localized only for presentation. Markdown export keeps English metadata, including its explicit date format. Language changes take effect when the app reopens, as explained beside the picker. Existing sessions continue in their launch language; changing the preference never restarts capture or rewrites saved content. `AppLanguageSettings` owns this preference independently of AI settings and the vocabulary editor; native bundles still resolve all translations at launch.
...
Turning off AI synchronously asks app assembly to cancel the coordinator’s insight, refinement/title, and meeting vocabulary owners, plus personal vocabulary extraction and settings service checks. Cancellation preserves local work and retained results. `AISettings.isAvailable` combines the master switch with connection validity; `isConfigured` includes connection, context-window, and credential-storage validity.
...
- [`AISettingsController.swift`](../Sources/AI/AISettingsController.swift): direct bindings to persisted preferences and cancellable connection-test/model-discovery state. Settings dismissal invalidates checks; preferences are already saved.
...
- [`GeneralSettingsView.swift`](../Sources/App/GeneralSettingsView.swift): language picker and AI master switch, both with native title/subtitle labels. [`AppLanguageSettings.swift`](../Sources/App/AppLanguageSettings.swift) owns the native per-app language preference.
- [`DefaultInsightSettings.swift`](../Sources/Insights/DefaultInsightSettings.swift): locally persisted reusable insight content shared by Settings and the coordinator. New drafts copy value templates into independently owned definitions; existing meetings never resync. The shared insight editor commits complete items explicitly.
- [`AISettingsView.swift`](../Sources/AI/AISettingsView.swift): renders automatically saved AI preferences, inline validation, model discovery, and connection testing.
...
- Settings is a native sheet with General, AI Services, Vocabulary, and Insights tabs. General is initially selected and contains App Language and the Enable AI Services switch. Both place their explanations inside native title/subtitle labels. AI address/model/context-window fields use the same label structure. Standalone and compound controls use native `LabeledContent`, including insight automatic generation, vocabulary text entry, Context attachments, and preparation vocabulary/insight actions. Inline errors, progress, and section-wide status retain their own presentation. General preferences save immediately; only language changes require quitting and reopening the app. When AI is off, AI Services keeps the saved configuration visible but disables every field, picker, and action, with guidance to enable AI in General. AI entry points open General when AI is off and AI Services when it is on. AI preferences save as edited through AISettings; SettingsNavigation owns only their service-check controller, with no connection draft or Save/Cancel transaction. All four tabs use native grouped forms with system section headers, neutral surfaces, standard controls, and fixed action footers. The Vocabulary tab directly embeds the shared editor without a repeated page heading; its saved-term section identifies the scope and its Done footer only dismisses. There is no Manage landing page, independent window, or nested vocabulary sheet. Configure AI Services switches tabs in the same 600×540 Settings sheet. Choosing files stays local; Extract explicitly sends text. Both vocabulary hosts preserve manual edits, suggestions, reading state, and background extraction on dismissal. Suggested Terms owns review actions and feedback; vocabulary commits never submit another draft. Vocabulary sections have independent observation boundaries, and only uniform 36-point term rows are lazy within the form’s single scroll container. Their fixed geometry preserves reading position while keeping large-list tab changes bounded by visible content.
