...
- 可在设置的“词表”Tab 编辑并持久化英文产品名、人名和缩写，提高本地识别命中率，并在会后优化时约束专有词拼写。
...
1. 如需维护英文识别与会后优化词表，打开设置（⌘,）的“词表”Tab，每行填写一个词或短语并保存。
...
音频捕获、语音识别、Apple Translation 和会议历史均在本机处理，应用不保存音频。AI 能力不属于离线主链路：当用户配置并触发服务后，会议文字会发送给所选第三方 Structured Outputs 接口；会后优化还会发送当前已保存词表，音频不会由该链路上传。
