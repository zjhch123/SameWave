...
For AI features, configure your provider, API key, and model context window in **Settings → AI Services**. Preferences save automatically as you edit, including while AI is off. **Enable AI Services** stops or enables AI without clearing your configuration. Invalid input is shown inline and blocks new AI requests until corrected. **Done** closes Settings; there is no separate save or draft state. **Test Connection** checks the current configuration. See the [AI guide](doc/04-ai-insights-and-refinement.md) for provider requirements and insight controls.
