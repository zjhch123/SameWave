            ...
            Form {
                SwiftUI.Section {
                    ...
                    Text(draftConfig.isCustom
                         ? "API 地址可填写域名、带 /v1 的地址或完整 /chat/completions 地址，应用会自动补全。测试连接会用嵌套 JSON 结构验证模型；运行时仍会严格校验返回内容。会议文字及会后优化使用的已保存词表会发送到该服务，音频不会上传。"
                         : "智能洞察会将会议对话内容发送到你选择的服务商；会后优化还会发送当前已保存词表。字幕与实时翻译始终在本地进行，不受此设置影响。")
                        .font(.system(size: 11))
                        .foregroundStyle(CaptionsView.meta)
                }
